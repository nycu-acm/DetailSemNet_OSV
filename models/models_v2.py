...
class vit_models(nn.Module):
    ...
    def forward_features_wocls(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
    ...
    def forward_features_wocls_v2(self, x):
        x = self.patch_embed(x)
        x = x + self.pos_embed
    ...
    def forward_features_tl(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
    ...
    def forward_features_tl_all(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
    ...
    def forward_features_tl_all_v2(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
    ...
    def forward_features_tl_test(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        #print(x.shape, self.pos_embed.shape)
        x = x + self.pos_embed
        ...
        for i , blk in enumerate(self.blocks):
            if i == 9:
                x, save_images = self.token_learner.forward_test(x)
            x = blk(x)

        x = self.norm(x)
        x = x.mean(dim=1)
        return x, save_images
    
    def forward_features_tl_all_v2_test(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        #print(x.shape, self.pos_embed.shape)
        cls_tokens = self.cls_token.expand(B, -1, -1)
    ...
    def forward_features_depatch(self, x):
        x = self.patch_embed(x)
        B, S = x.shape[0], int(x.shape[1] ** 0.5)
    ...
    def forward_features_fg(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        if isinstance(x, tuple):
            x, (H, W) = x
        ...
        for i , blk in enumerate(self.blocks):
            if i < (total_len - 1):
                x = blk(x)
                attn_weights.append(blk.attn.get_attn_weight())
            else:
                part_num, part_inx = self.part_select(attn_weights) # need PSM
                #print(part_num, part_inx)
                part_inx = part_inx + 1
                parts = []
                ...
                x = torch.cat((x[:,0].unsqueeze(1), parts), dim=1)
                x = blk(x)
